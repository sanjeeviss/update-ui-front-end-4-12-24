...
export default function DesignationMasterForm1() {
  ...
  const [designationData, setDesignationData] = useState([{ vDesignationName: '', authority: '', status: '' }]);
    ...
    try {
      ...
      if (selectedBranchIds.length > 0) {
        ...
        selectedBranchIds.forEach((branchId) => {
          values.designationData.forEach((designation) => {
            const query = `
              INSERT INTO Paym_Designation (pn_CompanyID, BranchID, v_DesignationName, Authority, Status)
              VALUES ('${values.pnCompanyId}', '${branchId}', '${designation.vDesignationName}', '${designation.authority}', 
              CASE 
                WHEN '${designation.status}' = 'Active' THEN 'A' 
                ELSE '${designation.status}' 
              END)
            `;
      ...
      // If no branches are selected and the branch list is empty
        ...
        values.designationData.forEach((designation) => {
          const query = `
                       INSERT INTO Paym_Designation (pn_CompanyID, BranchID, v_DesignationName, Authority, Status)
            VALUES ('${values.pnCompanyId}', NULL, '${designation.vDesignationName}', '${designation.authority}', 
            CASE 
              WHEN '${designation.status}' = 'Active' THEN 'A' 
              ELSE '${designation.status}' 
            END)
  ...
  const handleAddDesignation = (designationData, setFieldValue) => {
    const updatedDesignationData = [
      ...designationData, 
      { vDesignationName: '', authority: '', status: 'Active' } // Default status 'A' for new rows
    ];
    setFieldValue('designationData', updatedDesignationData);
  };

  const handleRemoveDesignation = (index, values, setFieldValue) => {
    // Create a copy of the current designationData array
    const updatedDesignationData = [...values.designationData];

    // Remove the specific row by filtering out the index
    updatedDesignationData.splice(index, 1);
                        ...
                        initialValues={{
                          ...
                          designationData: designationData.length > 0
                            ? designationData.map((designation) => ({
                                vDesignationName: designation.vDesignationName || '',
                                authority: designation.authority || '',
                                status: designation.status || 'Active'
                              }))
                            : [{ vDesignationName: '', authority: '', status: 'Active' }]
                        }}
                        ...
                        {({ values, handleChange, handleBlur, errors, touched, resetForm, setFieldValue }) => (
                          <Form>
                            <Grid container spacing={2}>
                              ...
                              {values.designationData.map((designation, index) => (
                                <Grid item xs={12} key={index}>
                                  <Grid container spacing={2} alignItems="center">
                                    <Grid item xs={3}>
                                      <FormControl fullWidth error={touched.designationData && touched.designationData[index] && Boolean(errors.designationData && errors.designationData[index] && errors.designationData[index].vDesignationName)}>
                                        <TextField
                                          name={`designationData.${index}.vDesignationName`}
                                          label={<span>Designation Name <span style={{ color: 'red', marginLeft: '0.2rem' }}>*</span></span>}
                                          variant="outlined"
                                          fullWidth
                                          value={designation.vDesignationName}
                                          onChange={handleChange}
                                          onBlur={handleBlur}
                                          InputLabelProps={{ shrink: true }}
                                        />
                                        {(touched.designationData && touched.designationData[index] && errors.designationData && errors.designationData[index] && errors.designationData[index].vDesignationName) && (
                                          <FormHelperText sx={{ color: 'error.main' }}>
                                            {errors.designationData[index].vDesignationName}
                                          </FormHelperText>
                                        )}
                                      </FormControl>
                                    </Grid>
                                    <Grid item xs={3}>
                                      <FormControl fullWidth error={touched.designationData && touched.designationData[index] && Boolean(errors.designationData && errors.designationData[index] && errors.designationData[index].authority)}>
                                        <TextField
                                          name={`designationData.${index}.authority`}
                                          label={<span>Authority <span style={{ color: 'red', marginLeft: '0.2rem' }}>*</span></span>}
                                          variant="outlined"
                                          fullWidth
                                          ...
                                          InputLabelProps={{ shrink: true }}
                                        />
                                        {(touched.designationData && touched.designationData[index] && errors.designationData && errors.designationData[index] && errors.designationData[index].authority) && (
                                          <FormHelperText sx={{ color: 'error.main' }}>
                                            {errors.designationData[index].authority}
                                          </FormHelperText>
                                        )}
                                      </FormControl>
                                    </Grid>
                                    <Grid item xs={3}>
                                      <FormControl fullWidth error={touched.designationData && touched.designationData[index] && Boolean(errors.designationData && errors.designationData[index] && errors.designationData[index].status)}>
                                        <TextField
                                          name={`designationData.${index}.status`}
                                          ...
                                          InputLabelProps={{ shrink: true }}
                                        />
                                        {(touched.designationData && touched.designationData[index]                                         && errors.designationData && errors.designationData[index] && errors.designationData[index].status) && (
                                          <FormHelperText sx={{ color: 'error.main' }}>
                                            {errors.designationData[index].status}
                                          </FormHelperText>
                                        )}
                                      </FormControl>
                                    </Grid>
                                    <Grid item xs={3} style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
                                      {values.designationData.length > 1 && (
                                        <Button
                                          type="button"
                                          variant="contained"
                                          color="secondary"
                                          onClick={() => handleRemoveDesignation(index, values, setFieldValue)}  // Pass the form values and setFieldValue
                                ...
                                <Grid container spacing={2} justifyContent="flex-end">
                                  <Grid item>
                                    <Button
                                      variant="contained"
                                      color="primary"
                                      onClick={() => handleAddDesignation(values.designationData, setFieldValue)}  // Updated function
