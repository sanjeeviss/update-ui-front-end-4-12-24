...
import { Button, CardContent, FormControlLabel, Grid, Switch,Typography } from '@mui/material';
...
const Android12Switch = styled(Switch)(({ theme }) => ({
  ...
  const AllowanceValues = () => {
    ...
    const [columnDefs, setColumnDefs] = useState([
        ...
        { headerName: "Values", field: "values", editable: true, flex: 1 ,  cellStyle: { textAlign: 'right' }},
    ...
    useEffect(() => {
        async function getData() {
          ...
          const GradeSlabdata = await postRequest(ServerConfig.url, REPORTS, {
            "query" : `select * from GradeSlab_Branch where pn_companyid = ${Company.data[0].pn_CompanyID} and pn_branchid = ${Branchid}`
          });
            ...
            gridData.forEach((row, index) => {
                ...
                if (existingRow) {
                    ...
                    const insertQuery = 
                        ...
                        VALUES
                        (${company[0].pn_CompanyID}, ${Branchid}, ${gradeField === 'Grade_Name' ? `'${gradeValue}'` : 'NULL'},
