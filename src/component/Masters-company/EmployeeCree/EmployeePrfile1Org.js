      ...
      try {
        ...
        if (isloggedin) {
          ...
          if (loggedCompanyData.data) {
            ...
            const loggedBranch = await postRequest(ServerConfig.url, REPORTS, {
              query: `select * from paym_branch where pn_CompanyID = ${loggedBranch.data[0].pn_CompanyID}`,
            });

            if (loggedBranch.data) {
              setloggedBranch(loggedBranch.data);
              setPnBranchId(loggedBranch.data[0].pn_CompanyID);
            }
          }
        }
      } catch (error) {
...
export default Employeeprofile0909h1;
