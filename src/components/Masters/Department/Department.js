...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const query = `
      ...
      VALUES (${formData.pn_CompanyID}, ${formData.pn_BranchID}, '${formData.v_DepartmentName}', '${formData.status.toUpperCase()}');
    `;
    ...
    if (response.data === 1 || response.success) {
      ...
      alert("Department added successfully!"); // Show alert for successful submission
      navigate("/DepartmentHome");
    } else {
      console.error("Failed to add Department. Response:", response); // Log the failure response
      alert("Failed to add Department.");
    }
  } catch (error) {
    console.error("Error adding Department:", error); // Log the error
